...
It is a difficult and controversial question how to dichotomize or convert data in any other scale ([discussion about controversion](http://allenfleishmanbiostatistics.com/Articles/2011/12/9-dichotomization-as-the-devils-tool/). In psychological or biological studies one of the possible ways is to split data at the median <https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1458573/>. I haven't good enough judgment about trades for choosing the most appropriate way, and I think it depends on our goals. If we are interested in question "do countries establish the most productive trade" we probably have to use *trade.max* dichotomization. If we are interested in question "do countries establish at least one trade in principle", we can use *trade.any* dichotomization.  
...
The most obvious difference is that these graphs differ from each other in the number of ties. That is quite logical because during dichotomizing we provided our matrices different amount of ones and zeros. It means that our networks will have all different centrality metrics.   
So, justification and choosing an appropriate number of ties according to the purpose of the research still seems reasonable.
...
If this question relates to general differences, the answer was done earlier (about different amount of ties and different centralities for it)
...
If I understood the question right, it is about centralization indexes. These numbers mean level of centralization of network, how strong network is revolved around one node. The larger centralization index is the more likely that a particular single node in the center of the network and the remaining nodes are far away from the center. Numbers differ from each other because they are based on different centrality metrics and are calculated differently.    
...
```{r minerals desciptive stat, echo = TRUE, include=TRUE}
...
These data provide us a general information about the network: how many contries are involved, how many connection in minerals trades they established, how these connections distributed per contries. Looking at the density, we can say, that in general countries trade minerals not so actively.
...
min.degreedf <- data.frame(min.indegree, min.outdegree, min.fulldegree)
min.degreedf <- cbind(as.data.frame(minerals)[0], min.degreedf) 
min.degreedf
```

Here we can see how many trade connections each country has. We can say that several countries receive more minerals than send (Egypt, Yugoslavia), some countries sell a lot of minerals and buy it a little (China, Japan, United Kindom, United States). Maybe this is because they have own resources.   


```{r minerals betweennes}
...
min.betwdf <- cbind(as.data.frame(minerals)[0], min.between) 
min.betwdf

```

Here we can see how many times each country connect another in mineral trades and acts as a bridge in this trade. From the histogram we see that majority of countries has small betweenness and there are some outliers with high values. The biggest betweenness -- United States. Probably this country not only actively buy minerals but also sell it to other countries and plays a very important role in trade. We also see that United Kindom, Egypt, Japan, Spain, and Yugoslavia has big betweenness index but no so big as US.    


```{r minerals closeness}
#closeness
...
min.closedf <- cbind(as.data.frame(minerals)[0], min.closeness) 
min.closedf
```
Here we can see the standartized sum of shortest paths between each country and other countries. As we can see from the histogram the majority of countries has middle value of closeness. The biggest closeness --  United States and United Kindoms. So, hypothesis about active involvmenet of US in minerals trade seems plausible.

```{r minerals eigen}
#eigenvectors
min.eigen <- evcent(minerals.net, gmode = 'digraph', diag = FALSE, rescale = FALSE)
min.eigen
summary(min.eigen)
hist(min.eigen)
min.eigendf <- cbind(as.data.frame(minerals)[0], min.eigen) 
min.eigendf
```
Here we can analyze which countries are more influential in terms of mineral trades. As expected, there is small amount of powerful countries and with decreasing of influence amount of countries increases. The more influential countries, again, are United States and United Kindom.
...
Centralization index shows how central this network or how strong all countries are revolved around one of them. This index not so big, so, we can conclude that minerals trade network rather decentralized.
